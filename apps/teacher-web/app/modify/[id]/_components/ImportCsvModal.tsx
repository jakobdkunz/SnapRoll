    ...
    <Modal open={open} onClose={onClose}>
      <div className="w-[min(92vw,900px)] bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 rounded-xl shadow-xl p-4 sm:p-6 transition-all duration-200 ease-out">
        {mappingStep === 'promptKind' && promptColumnIdx != null && (
          <div>
            <div className="mb-1 text-xs font-semibold tracking-wide text-slate-500 dark:text-slate-400">CSV Import</div>
            ...
            <div className="mb-4 flex justify-center">
              <div className="w-[min(520px,88vw)]">
                <div className="text-xs text-slate-500 dark:text-slate-400 mb-1 text-center">{csvColumns[promptColumnIdx] || `Column ${promptColumnIdx + 1}`}</div>
                {(() => {
        ...
        {mappingStep === 'review' && (
          <div>
            ...
            <div className="text-sm text-slate-600 dark:text-slate-300 mt-1 mb-4">Adjust each column as needed before importing.</div>
                      ...
                      <th key={i} className="text-left px-2 py-2 border-b border-slate-200 dark:border-slate-700 whitespace-nowrap align-bottom">
                        ...
                        <div className="relative inline-block">
                          ...
                          >
                            ...
                            <HiChevronDown className="absolute right-1.5 top-1.5 h-3.5 w-3.5 text-slate-500 dark:text-slate-400 pointer-events-none" />
                          </button>
                          {openDropdownIdx === i && (
            ...
            <div className="flex items-center justify-end gap-3 pt-3">
              {!validateRolesForImport(columnRoles).ok && (
                <div className="text-xs text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 rounded px-2 py-1">
                  {(() => {
